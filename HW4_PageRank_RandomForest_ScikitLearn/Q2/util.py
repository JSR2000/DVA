...
def entropy(class_y):
    ...
    u_classes, u_counts = np.unique(arr, return_counts=True)
    entropy = stats.entropy(u_counts, base=2)
    return entropy
...
def information_gain(previous_y, current_y):
    ...
    cy_l = current_y[0]
    cy_r = current_y[1]
    entropy_left = float(len(cy_l))/float(lnth)*entropy(cy_l)
    entropy_right = float(len(cy_l))/float(lnth)*entropy(cy_r)
    info_gain = entropy_prev - entropy_left - entropy_right
    return info_gain
